...
{
//    isBypassed = m_pAPVTS->getRawParameterValue("SGAmpBypass"+suffix)->load();
//
//    inputgain.setTargetValue(juce::Decibels::decibelsToGain (m_pAPVTS->getRawParameterValue("SGAmpGain"+suffix)->load()));
//
//    ampMaster = inputgain.getNextValue();
}

void CSmartGuitarAmp::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    DBG("----------inspector starts------------");
    DBG(samplesPerBlock);
    DBG(sampleRate);
    DBG("----------inspector ends--------------");
    waveNet.prepareToPlay(samplesPerBlock);
    juce::File default_tone("/Users/shanjiang/Documents/GitHub/SmartGuitarAmp/models/bluej_fullD_p0153.json");
